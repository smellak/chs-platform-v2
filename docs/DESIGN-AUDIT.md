...
| Archivo | Línea | Texto |
|---------|-------|-------|
| `src/lib/actions/api-keys.ts` | 31 | `chs_sk_{randomBytes}` — API key prefix |
| `src/app/api/api-keys/route.ts` | 48 | `chs_sk_{randomBytes}` — API key generation |
...
| Archivo | Línea | Texto |
|---------|-------|-------|
| `src/lib/traefik-manager.ts` | 21 | `"chs-app-{appSlug}.yaml"` — config filename |
| `src/lib/traefik-manager.ts` | 25 | `"chs-forward-auth.yaml"` — auth config |
| `src/lib/traefik-manager.ts` | 40 | `"Auto-generated by CHS Platform Platform"` — comment |
| `src/lib/traefik-manager.ts` | 165 | `resolveAlephVerifyUrl()` — function name |
| `src/lib/traefik-manager.ts` | 166 | `process.env["CHS_INTERNAL_URL"]` — env var |
